...
class QueryAdcodeHandler(Handler):
    ...
    def handle(self, arg: List) -> AlfredResult:
        ...
        try:
            ...
            # 检查响应
            if data.get("code")!=0:
                return AlfredResult.one("", f"查询失败: {data.get('message')}", "请检查输入")
            ...
            for item in results:
                ...
                subtitle = f"级别: {item['level_type']} | 经纬度: {item['lng']},{item['lat']}"
                items.append(AlfredResultItem(arg=arg_value, title=title, subtitle=subtitle))
